...
class RenderingConfig(BaseModel):
    ...
    # When on: after a host tool sends an image, the turn's remaining narration is
    # folded (edit-in-place) into a placeholder bubble dropped right after the image,
    # instead of appended as a new message below any reply the user slips in during
    # the gap. Keeps causal order without moving the user's message. Off by default
    # (opt-in) — the placeholder shows a brief "writing…" cue on every image.
