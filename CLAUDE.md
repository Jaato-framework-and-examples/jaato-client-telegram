...
- Idle detach is deliberate: the bot detaches idle chats (`__main__.py::_idle_session_cleanup_task`) so the server, which unloads a session only when it has **no attached clients** (orphaning — there is no server idle timer), frees the runner for active chats. The first message after a detach pays a **cold runner revive** (re-spawn + plugin re-bootstrap → "⏳ Initializing…", then `attach_session`/`_load_session` restores history under the same id → "⏳ Resuming…"). This per-chat-warmth-for-fleet-scale trade is intentional and **kept as-is** (warm-idle TTL was considered and declined, 2026-06-27). Full rationale: `docs/design/session-lifecycle.md`.
