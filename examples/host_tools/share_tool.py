...
async def execute(args: dict, ctx) -> dict:
    ...
    # It becomes a PUBLIC PR (code visible) — confirm with the user.
    ans = await ctx.ask(
    ...
    if ans != "Yes, open the PR":
        return {"result": "Cancelled — no PR opened."}
    ...
    st, existing = _gh("GET", f"/repos/{owner}/{_REPO}/contents/{path}?ref={branch}", token)
    file_sha = existing.get("sha") if st == 200 and isinstance(existing, dict) else None
